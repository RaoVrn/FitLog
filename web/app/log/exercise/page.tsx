...
import { Dumbbell, Plus, Trash2, Timer, Repeat, Flame, Loader2, CheckCircle } from "lucide-react";
          ...
          {EXERCISE_TEMPLATES.map((t) => (
            <button
              ...
              className={`rounded-lg px-3 py-1.5 text-sm ring-1 transition ${
                exerciseName === t.name
                  ? "bg-green-500 text-slate-950 ring-green-500"
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {exercises.map((exercise) => (
              <ExerciseItem key={exercise.id} exercise={exercise} onDelete={handleDelete} />
            ))}
          </div>
          <button
            onClick={handleSave}
